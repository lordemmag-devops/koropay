...
import { mockLevySettings } from '../../data/mock';
import type { LevySetting } from '../../types';

export default function LevySettings() {
  const [levies, setLevies] = useState<LevySetting[]>(mockLevySettings);
  ...
  const toggleActive = (id: string) => {
    setLevies(levies.map(l => l.id === id ? { ...l, active: !l.active } : l));
  };

  const deleteLevi = (id: string) => {
    setLevies(levies.filter(l => l.id !== id));
  };

  const startEdit = (levy: LevySetting) => {
    ...
    <div className="max-w-5xl mx-auto">
      ...
      >
        ...
        <div>
          ...
          <p className="text-xs text-surface-200/40">Levy amounts set here are used by all agents. Drivers and agents cannot change these prices.</p>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              ...
              <div>
                ...
                <input
                  ...
                  placeholder="e.g. Ojuelegba Checkpoint"
                  className="input-field"
                />
              </div>
            </div>
            <div className="flex gap-3">
              <button type="submit" className="btn-primary">Create Levy</button>
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-4">
                <div className={`w-11 h-11 rounded-2xl flex items-center justify-center ${
                  levy.active ? 'bg-emerald-500/15' : 'bg-surface-800'
                }`}>
                  {levy.active
                ...
                <div>
                  ...
                  {levy.location && (
                    <p className="text-xs text-surface-200/40 mt-0.5">{levy.location}</p>
                  )}
                </div>
              </div>
              ...
              <div className="flex items-center gap-4">
                {editingId === levy.id ? (
                  <div className="flex items-center gap-2">
                    ...
                    <button onClick={() => saveEdit(levy.id)} className="text-emerald-400 hover:text-emerald-300 transition-colors">
                      <Save className="w-4 h-4" />
                    </button>
                    <button onClick={() => setEditingId(null)} className="text-surface-200/30 hover:text-white transition-colors">
                      <X className="w-4 h-4" />
                    </button>
                  </div>
                ) : (
                  <>
                    ...
                    <button onClick={() => startEdit(levy)} className="text-surface-200/30 hover:text-primary-400 transition-colors">
