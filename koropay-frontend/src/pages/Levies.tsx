...
import { Shield, Wallet, AlertCircle, CheckCircle2, Clock, KeyRound } from 'lucide-react';
...
export default function Levies() {
  ...
  const handlePayNow = async (id: string) => {
    setRequesting(true);
    try {
      const res = await driverApi.requestLevyOTP(id);
      setActiveOtp({ id, otp: res.otp });
      setOtpInput('');
      setOtpError(false);
    } catch (err: any) {
      alert(err.message);
  ...
  const handleVerify = async () => {
    ...
    setVerifying(true);
    try {
      await driverApi.verifyLevyOTP(activeOtp.id, otpInput);
      setActiveOtp(null);
      setOtpInput('');
      setOtpError(false);
      fetchLevies();
              ...
              <div key={p.id} className="p-5">
                <div className="flex items-center justify-between mb-3">
                  <div>
                    ...
                    <p className="text-xs text-surface-200/40 mt-0.5">{p.agent?.user?.name ?? p.agentId}</p>
                  </div>
                ...
                <AnimatePresence mode="wait">
                  {activeOtp?.id === p.id ? (
                    <motion.div key="otp" initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} className="space-y-3">
                      ...
                      {otpError && <p className="text-xs text-rose-400 text-center">Invalid OTP</p>}
                    </motion.div>
                  ) : (
                    <motion.button
                      key="pay"
                      ...
                      disabled={requesting}
                      className="w-full btn-primary flex items-center justify-center gap-2 text-sm py-2.5 disabled:opacity-60"
                    >
                      <Wallet className="w-4 h-4" /> {requesting ? 'Sending OTP...' : 'Pay Now'}
                    </motion.button>
                  )}
                </AnimatePresence>
              </div>
              ...
              <div key={p.id} className="px-6 py-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-surface-200/40">{p.agent?.user?.name ?? p.agentId}</p>
                  </div>
                </div>
